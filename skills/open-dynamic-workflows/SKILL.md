...
const results = await pipeline(
  DIMENSIONS,
  (d) => agent(d.prompt, { executor: 'zcode', label: `review:${d.key}`, phase: 'Review', schema: FINDINGS_SCHEMA }),
  (review) =>
    parallel(
      review.findings.map((f) => () =>
        agent(`Adversarially verify this finding — is it real? ${f.title}`, {
          executor: 'zcode', label: `verify:${f.file}`, phase: 'Verify', schema: VERDICT_SCHEMA,
        }).then((v) => ({ ...f, verdict: v })),
      ),
    ),
)
...
**Cursor** (`./scripts/install-cursor-cli.sh` then `agent mcp enable open-dynamic-workflows`,
or Customize / `~/.cursor/plugins/local`): omitted `executor` runs on **cursor**. Name `grok` /
`zcode` / `claude` / `codex` to use another CLI.
